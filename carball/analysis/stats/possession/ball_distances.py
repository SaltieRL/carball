...
from ....analysis.stats.utils.pandas_utils import sum_deltas_by_player_name, sum_deltas
...
class BallDistanceStat(BaseStat):
    ...
    def calculate_player_stat(self, player_stat_map: Dict[str, PlayerStats], game: Game, proto_game: game_pb2.Game,
        ...
        for player_id in player_stat_map.keys():
            ...
            time_close_to_ball = player_distance_with_delta[close_frames]['delta'].sum() / time_elapsed
            distance_stats = player_stat_map[player_id].distance

            distance_stats.time_close_to_ball = time_close_to_ball

            try:
                distance_stats.time_closest_to_ball = player_distance_times['closest_player'][player_id] / time_elapsed
                distance_stats.time_furthest_from_ball = player_distance_times['furthest_player'][
                                                             player_id] / time_elapsed
    ...
    def calculate_player_distance_to_location(player_map: Dict[str, Player], data_frame: pd.DataFrame,
        ...
        player_distance_with_delta = pd.concat(
            [player_distances_data_frame, data_frame['game', 'delta'].rename('delta')], axis=1)
